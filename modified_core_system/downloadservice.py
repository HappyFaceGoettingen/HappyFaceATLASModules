...
class DownloadSlave(threading.Thread):
    ...
    def run(self):
        try:
            ...
            elif self.file.url.startswith("http://bigpanda.cern.ch/jobs") or self.file.url.startswith("http://bigpanda.cern.ch/tasks"):
                '''
...
wget --quiet --no-cookies --referer="$ref" --user-agent="$agent" --header="$header1" --header="$header2" --header="$header3" --header="$header4" --header="$header5" --header="$cookie_header" "$url" -O $filename
                '''
                command = "wget --header='Accept: application/json' --header='Content-Type: application/json' --output-document=\"%s\" %s %s \"%s\"" % (self.file.getTmpPath(True), "" if self.file.config_source == "local" else self.global_options, self.file.options, self.file.url)
