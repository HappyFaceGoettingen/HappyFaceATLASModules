...
class DDM2(hf.module.ModuleBase):
    config_keys = {
        'site_name': ('Site name', 'GOEGRID'),
        'cloud': ('Cloud', 'DE'),
        'time_interval': ('Time interval in minutes', '1200'),
	'url_destination_space_tokens': ('URL for the destination space tokens', 'local||http://dashb-atlas-ddm.cern.ch/dashboard/request.py/matrix.json?src_grouping=cloud&dst_cloud=%%22%s%%22&dst_site=%%22%s%%22&dst_grouping=cloud&dst_grouping=site&dst_grouping=token&interval=%s'),
        'url_source_space_tokens': ('URL for the source space tokens', 'local||http://dashb-atlas-ddm.cern.ch/dashboard/request.py/matrix.json?&src_grouping=cloud&src_cloud=%%22%s%%22&src_site=%%22%s%%22&dst_grouping=cloud&src_grouping=site&src_grouping=token&interval=%s'),
	...
	# reading in beginning and end of interval from json
        json_content2 = json.loads(content_destination_space_tokens)

	data['start_time'] = json_content2['params']['from_date']
	data['end_time'] = json_content2['params']['to_date']
